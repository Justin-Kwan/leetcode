class Solution:
    # optimal single search approach
    def search(self, nums: List[int], target: int) -> int:
        ...
        # keep guiding left right pointers into same side as target
        while left <= right:
            middle = (left + right) // 2
            # target is in left and middle currently in right
            if target >= nums[0] and nums[middle] < nums[0]:
                right = middle - 1
            # target is in right and middle currently in left
            elif nums[middle] >= nums[0] and target < nums[0]:
                left = middle + 1
            # target and middle both in same side so binary search and
            # move up left and right pointers normally
            else:
        ...
        # target not found once left right pointers crossed
        return -1
